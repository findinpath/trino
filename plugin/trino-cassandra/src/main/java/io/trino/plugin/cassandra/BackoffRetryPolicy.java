        ...
        implements RetryPolicy
{
    ...
    public RetryDecision onReadTimeout(Request request, ConsistencyLevel cl, int blockFor, int received, boolean dataPresent, int retryCount)
    {
        ...
        if (decision == RetryDecision.RETRY_SAME) {
            log.debug(
                    "[%s] Retrying on read timeout on same host (consistency: %s, required responses: %s, "
                    + "received responses: %s, data retrieved: %s, retries: %s)",
                    logPrefix,
                    cl,
                    blockFor,
                    received,
    ...
    public RetryDecision onWriteTimeout(Request request, ConsistencyLevel cl, WriteType writeType, int blockFor, int received, int retryCount)
    {
        ...
        if (decision == RetryDecision.RETRY_SAME && log.isDebugEnabled()) {
            log.debug(
                    "[%s] Retrying on write timeout on same host (consistency: %s, write type: %s, "
                            + "required acknowledgments: %s, received acknowledgments: %s, retries: %s)",
                    logPrefix,
                    cl,
                    writeType,
                    blockFor,
