        ...
        extends AbstractTestQueryFramework
{
    ...
    {
        // Exercises the vended credentials used to write the table's initial data files at create time, and to read them back
    ...
    {
        // Exercises the vended credentials used for all data modification statements against an already-committed table
        try (TestTable table = newTrinoTable("test_dml", "(id bigint, value varchar)")) {
            ...
            assertQuery("SELECT * FROM " + tableName, "VALUES (1, 'INDIA'), (2, 'POLAND')");

            assertUpdate("UPDATE " + tableName + " SET value = 'FRANCE' WHERE id = 1", 1);
            assertQuery("SELECT * FROM " + tableName, "VALUES (1, 'FRANCE'), (2, 'POLAND')");

            assertUpdate("DELETE FROM " + tableName + " WHERE id = 2", 1);
            assertQuery("SELECT * FROM " + tableName, "VALUES (1, 'FRANCE')");
