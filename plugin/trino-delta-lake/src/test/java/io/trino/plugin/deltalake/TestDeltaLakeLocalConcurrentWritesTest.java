        ...
        extends AbstractTestQueryFramework
{
    ...
    @Test
    public void testConcurrentMixedInsertsSelectingFromTheSameTableAndBlindInserts()
            throws Exception
    {
        int threads = 3;
        CyclicBarrier barrier = new CyclicBarrier(threads);
        ExecutorService executor = newFixedThreadPool(threads);
        String tableName = "test_concurrent_mixed_inserts_select_from_same_table_and_blind_inserts_" + randomNameSuffix();

        assertUpdate("CREATE TABLE " + tableName + " (a) AS VALUES 0", 1);

        // Depending on the actual order of execution of the following queries we may have either of the following values in the table:
        // (0), (1), (10), (20)
        // (0), (2), (10), (20)
        // (0), (3), (10), (20)
        // Independently of the execution order of the statements, the INSERT operation selecting information from the same
        // table should complete successfully because there are no conflicts with blind inserts in the `WriteSerializable`
        // isolation level.
        try {
            executor.invokeAll(ImmutableList.<Callable<Void>>builder()
                            .add(() -> {
                            ...
                            .add(() -> {
                                barrier.await(10, SECONDS);
                                getQueryRunner().execute("INSERT INTO " + tableName + " VALUES 10");
                                return null;
                            })
                            .add(() -> {
                                barrier.await(10, SECONDS);
                                getQueryRunner().execute("INSERT INTO " + tableName + " VALUES 20");
                                return null;
                            })
                            .build())
                    .forEach(MoreFutures::getDone);

            assertQuery("SELECT sum(a) BETWEEN 31 AND 33 FROM " + tableName, "VALUES true");
            assertQuery(
                    "SELECT version, operation, isolation_level, read_version FROM \"" + tableName + "$history\"",
                    """
                            VALUES
